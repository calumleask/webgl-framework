...
export class Renderer {
  ...
  init(canvas: Canvas): void {
    ...
    const vertexArrayObject = this._createVAOAndSetAttributes(
      glContext,
      this._program.getProgramInfo().attribSetters,
      attribs,
    );
  ...
  private _createVAOAndSetAttributes(
    glContext: WebGL2RenderingContext,
      ...
      switch (attribName) {
        case 'a_position': {
          ...
          glContext.vertexAttribPointer(
            attribSetters[attribName].location,
            size,
            type,
            normalize,
            stride,
